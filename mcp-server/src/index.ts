...
function validateParticleFile(data: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
...
const TOOLS: Tool[] = [
  {
    ...
    description:
      'List all available particle effect examples in the Snowstorm project. Returns the names of example particle files that can be used as templates.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description:
      'Read and parse a particle effect file. Accepts either an example name (e.g., "fire") or a full file path. Returns the parsed JSON content of the particle file.',
    inputSchema: {
      type: 'object',
      properties: {
        filename: {
          type: 'string',
          description:
            'Name of the example particle (e.g., "fire", "snow") or full path to a particle file',
        },
      },
      required: ['filename'],
    },
  },
  {
    ...
    description:
      'Validate a particle effect configuration. Checks if the provided JSON contains all required fields for a valid Minecraft Bedrock Edition particle effect.',
    inputSchema: {
      type: 'object',
      properties: {
        content: {
  ...
  },
  {
    ...
    description:
      'Generate a basic particle effect template with the specified identifier. Creates a minimal valid particle configuration that can be customized.',
    inputSchema: {
      type: 'object',
      properties: {
        identifier: {
          type: 'string',
          description:
            'Identifier for the particle effect (e.g., "myproject:custom_particle")',
        },
        emitter_type: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Get information about available Minecraft particle components and their properties. Helps understand what components can be used in particle effects.',
    inputSchema: {
      type: 'object',
      properties: {
        component_name: {
  ...
  try {
    switch (name) {
      ...
      case 'create_particle_template': {
        ...
        const emitterType = (args?.emitter_type as string) || 'steady';
        ...
        let emitterComponent: Record<string, any> = {};
        let lifetimeComponent: Record<string, any> = {};
      ...
      case 'get_particle_components': {
        ...
        if (componentName) {
          const component = PARTICLE_COMPONENTS[componentName as keyof typeof PARTICLE_COMPONENTS];
          if (!component) {
            return {
              content: [
                {
